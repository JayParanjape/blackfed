...
for i in range(num_meta_epochs):
    for j in range(len(datasets_list)):
        ...
        server, clients[j] = train(server, clients[j], datasets[j], j, save_path='./saved_models3_dice_server_bg'+str(lr_server)+'_client'+str(lr_client)+'/'+str(datasets_list[j]), loss_string='dice', device=device, bs=32, lr_server=float(lr_server), lr_client=float(lr_client), ignore_idx=0)
