...
def train(server, client, dataset_dict, j, save_path, loss_string, device, bs=8, lr_server=1e-4, lr_client=0.005, ck=0.01, ignore_idx = -1, tmp_save_path = '.'):
    ...
    best_tr_loss = 1000000000
    server.train()
    for epoch in range(num_epochs_server2):
        running_loss = 0.0
        running_intersection = 0
        running_union = 0
        running_corrects = 0
        ...
        if epoch%5==0:
            ...
            if epoch_loss < best_val_loss:
                best_val_loss = epoch_loss
                torch.save(server.state_dict(), os.path.join(tmp_save_path,'server_best_val2.pth'))
            torch.save(client.state_dict(), os.path.join(tmp_save_path, 'client_current2_'+str(j)+'.pth'))
