...
if __name__ == '__main__':
    ...
    model = Mask2Former(n_channels=64, n_classes=19, config_path='../../mmsegmentation/configs/mask2former/cityscapes_blackfed.py')
    if fed_learning:
    ...
    if not only_test:
        ...
        model = train(model, dataset_dict, save_path = './saved_models_dice' , loss_string='dice', device=device, num_epochs=num_epochs, center_num=center_num, bs=8, lr=1e-3)
        model = test(model, dataset_dict, load_path = './saved_models_dice/client_'+str(int(center_num)-1)+'_best_val.pth', loss_string='dice', device=device)
    else:
