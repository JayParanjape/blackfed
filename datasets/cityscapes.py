...
class CITYSCAPES_Dataset(Dataset):
    ...
    def populate_lists(self):
        if self.is_train:
            ...
            if self.is_test:
                ...
                imgs_root = os.path.join(self.root_path,'leftImg8bit_trainvaltest','leftImg8bit','val')
                masks_root = os.path.join(self.root_path,'gtFine_trainvaltest','gtFine','val')
    ...
    def __getitem__(self, index):
        ...
        for i in range(num_labels):
            label_ohe[i] = torch.eq(label, torch.Tensor(palette[i]))[:,:,0]
