    ...
    for epoch in range(1,num_epochs+1):
        ...
        for inputs, labels,text_idxs, text in dataloaders['train']:
            ...
            with torch.no_grad():
                ...
                # print('range of val inputs: ',inputs.min(), inputs.max())
                # print('range of train labels: ',labels.min(), labels.max())
                # print('range of train outputs: ',outputs.min(), outputs.max())
        ...
        if epoch%5==0:
            ...
            for inputs, labels,text_idxs, text in dataloaders['val']:
                ...
                with torch.no_grad():
                    ...
                    running_iou += no_blank_miou(labels, preds)
                    # print('range of val inputs: ',inputs.min(), inputs.max())
                    # print('range of val labels: ',labels.min(), labels.max())
                    # print('range of val outputs: ',outputs.min(), outputs.max())
...
if __name__ == '__main__':
    ...
    model = UNext(n_channels=64, n_classes=20)
    if fed_learning:
    ...
    if not only_test:
        ...
        model = train(model, dataset_dict, save_path = './saved_models3_dice' , loss_string='dice', device=device, num_epochs=num_epochs, center_num=center_num, bs=32)
