...
#             try:
#                 clients[datasets_list[j]-1].load_state_dict(torch.load('./saved_models5_server51client5_rev/tmp_client_bg'+str(datasets_list[j])+'.pth'))
#             except:
#                 pass
...
#         # if i>0 and i%2==0:
#         #     ck = ck/2
#         server, clients[datasets_list[j]-1] = train(server, clients[datasets_list[j]-1], datasets[j], datasets_list[j]-1, save_path='./saved_models5_server51client5_rev/'+str(datasets_list[j]), loss_string='dice', device=device, bs=9, lr_client=lr_client, ck=ck, tmp_save_path='./saved_models5_server51client5_rev', num_epochs_client=5, num_epochs_server=51)
