...
if __name__ == '__main__':
    ...
    model = UNext(n_channels=64, n_classes=21)
    if fed_learning:
    ...
    if not only_test:
        ...
        model = train(model, dataset_dict, save_path = './saved_models3_dice' , loss_string='dice', device=device, num_epochs=num_epochs, center_num=center_num, bs=32, lr=1e-4, ignore_idx=0)
        model = test(model, dataset_dict, load_path = './saved_models3_dice/client_'+str(int(center_num)-1)+'_best_val.pth', loss_string='dice', device=device, ignore_idx=0)
    else:
        ...
        model = test(model, dataset_dict, load_path = load_path, loss_string='dice', device=device, center_num=center_num, ignore_idx=0)
