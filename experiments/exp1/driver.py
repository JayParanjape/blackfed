...
for i in range(num_meta_epochs):
    for j in range(len(datasets_list)):
        ...
        server, clients[j] = train(server, clients[j], datasets[j], j, save_path='./saved_models_2/'+str(datasets_list[j]), loss_string='bce + dice', device='cuda:5' )
        torch.cuda.empty_cache()

#testing
for j in range(len(datasets_list)):
    server.load_state_dict(torch.load('./tmp_server.pth'))
    clients[j].load_state_dict(torch.load('./saved_models_2/'+str(datasets_list[j])+'/client_best_val.pth'))
    test(server, clients[j], datasets[j], device='cuda:5')
