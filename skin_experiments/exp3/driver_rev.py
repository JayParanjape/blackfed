...
#testing
for j in range(len(datasets_list)):
    # server.load_state_dict(torch.load('./safe/saved_models_rev/tmp_server_bg.pth'))
    server.load_state_dict(torch.load('./safe/saved_models_rev/'+str(datasets_list[j])+'/server_best_val.pth'))
    server.eval()
    for k in range(len(datasets_list)):
        ...
        # except:
        clients[j].load_state_dict(torch.load('./safe/saved_models_rev/tmp_client_bg'+str(datasets_list[j]-1)+'.pth'))
        clients[j].eval()
